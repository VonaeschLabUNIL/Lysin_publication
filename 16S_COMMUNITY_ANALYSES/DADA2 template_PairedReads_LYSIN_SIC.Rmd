...
setwd("/Users/julian/Documents/1A_POST_DOC_LAUSANNE_2022/14_ALINE_MASTER/LYSIN_IN_SIC_SEQUENCING/X204SC23094416-Z01-F013_16SV34/result_X204SC23094416-Z01-F013/01.RawData/SEQUENCES")

# Save path to fastq file folder after unzipping
path <- "/Users/julian/Documents/1A_POST_DOC_LAUSANNE_2022/14_ALINE_MASTER/LYSIN_IN_SIC_SEQUENCING/X204SC23094416-Z01-F013_16SV34/result_X204SC23094416-Z01-F013/01.RawData/SEQUENCES"
# Control files
list.files(path)
```
...
sample.namesF<- sapply(strsplit(basename(fnFs), "_1.fastq.gz"), `[`, 1)
sample.namesR <- sapply(strsplit(basename(fnRs), "_2.fastq.gz"), `[`, 1)
...
samples_to_keep <- track[,4] > 100 #your threshold. try different ones to get the lowest one that will work. 
...
track <- cbind(sapply(derepFs[samples_to_keep], getN),
                   sapply(derepRs[samples_to_keep], getN),
...
write.table(seqtab, "seqtab_2.txt", sep="\t", quote=F, row.names=T, col.names=T)
...
################################################################################################
########## OR IF FILES HAVE BEEN SAVED AS TABLE (TSV) AND ARE NOT R OBJECTS ANYMORE
################################################################################################
...
# Read the TSV files into R
seq_table1 <- read.table("path_to_your_first_file.tsv", header = TRUE, sep = "\t", row.names = 1)
seq_table2 <- read.table("path_to_your_second_file.tsv", header = TRUE, sep = "\t", row.names = 1)

# Convert the data frames to matrix format (required by DADA2)
seq_table1 <- as.matrix(seq_table1)
seq_table2 <- as.matrix(seq_table2)

# Merge the sequence tables
seqtab <- mergeSequenceTables(seq_table1, seq_table2)

# Optionally, save the merged sequence table to a file
write.table(merged_seq_table, "merged_sequence_table.tsv", sep = "\t", col.names = NA, quote = FALSE)

# Print the merged sequence table to check
print(merged_seq_table)

################################################################################################
# THEN YOU CAN GO ON WITH THE REST OF THE PROCEDURE
################################################################################################
...
# Save sequence table
write.table(seqtab.nochim, "sequence_table.txt", sep="\t", quote=F, row.names=T, col.names=T)
```
...
write.table(track, "read_retention_table.txt", quote=F, row.names=T, col.names=T, sep="\t")
...
taxa <- assignTaxonomy(seqtab.nochim, "/Users/julian/Bioinfo_Local/SILVA_DATABASE_STUFF/silva_nr99_v138.1_wSpecies_train_set.fa.gz", multithread=TRUE)
...
# Save taxonomy table
write.table(taxa, "taxonomy.txt", sep="\t", quote=F, row.names=T, col.names=T)
